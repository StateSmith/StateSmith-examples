...
static void STATE_1_do(MySm* self);

static void STATE_2_enter(MySm* self);
static void STATE_2_exit(MySm* self);
static void STATE_2_ev2(MySm* self);
static void STATE_2_myev1(MySm* self);
...
static void STATE_1_enter(MySm* self)
{
    ...
    self->current_event_handlers[MySm_EventId_DO] = STATE_1_do;
}
...
static void STATE_2_enter(MySm* self)
{
    ...
    self->current_event_handlers[MySm_EventId_EV2] = STATE_2_ev2;
    self->current_event_handlers[MySm_EventId_MYEV1] = STATE_2_myev1;
}
...
static void STATE_2_exit(MySm* self)
{
    ...
    self->current_event_handlers[MySm_EventId_MYEV1] = NULL;  // no ancestor listens to this event
}
...
static void STATE_2_ev2(MySm* self)
{
    ...
    // uml: EV2 / { printf("ev2 stuff"); }
    {
        ...
        // Step 1: execute action `printf("ev2 stuff");`
        printf("ev2 stuff");
...
static void STATE_2_myev1(MySm* self)
{
    ...
    // STATE_2 behavior
    // uml: myEv1 / { printf("myEv1 stuff"); }
    {
        // note: no ancestor consumes this event, but we output `bool consume_event` anyway because a user's design might rely on it.
        bool consume_event = true; // events other than `do` are normally consumed by any event handler. Other event handlers in *this* state may still handle the event though.
        (void)consume_event; // avoid un-used variable compiler warning. StateSmith cannot (yet) detect if behavior action code sets `consume_event`.
        
        // Step 1: execute action `printf("myEv1 stuff");`
        printf("myEv1 stuff");
